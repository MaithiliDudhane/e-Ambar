<?php
 ...
 $query = "SELECT * FROM uploadenglishpoem WHERE id='$id'";
...
<a href="viewengpoemdata.php">
           ...
           <div>
              ...
              accept="image/*" name="newimg">
              <input type="hidden" id="fname" name="oldimg" value="<?php echo $result['image'];?>">
            </div>
            <?php echo "<img src= 'uploadengpoemimg/".$result['image']."' height='100px' width='120px' >";?>
            <div>
            <label for="Poem">Poem</label>
            <textarea id="Poem" name="poem">
            <?php echo $result['englishpoem'];?>
            </textarea>
            </div>
           <button class="btn button" style="font-size: 19px; padding: 10px 25px; margin: 1px;margin-top: 2%;" 
           role="button" name="editEpoem">Edit English Poem</button> 
...
if(isset($_POST['editEpoem']))
{
   ...
   else
   {
    ...
    if($poemname !="" && $fname !="" && $lname !=""  && $year !="" && $dept !="" && $filename !="" && $Epoem !="")
{
    $query = "UPDATE uploadenglishpoem SET poemname='$poemname',firstname='$fname',lastname='$lname',yearofstudy='$year',
    department='$dept',englishpoem='$Epoem' WHERE id='$id'";
    ...
    if($data)
    {
      ...
      ?>
         <meta http-equiv = "refresh" content = "0; url = uploadedenglishpoem.php"/>
      <?php
  }
  else
  {        
