<?php
 ...
 $query = "SELECT * FROM uploadphography WHERE id='$id'";
					...
					<div class='container' style="padding:15px 56px 38px 310px;;">
						<div class='card'>
						   ...
						   <div>
							 ...
							 <input type="hidden" id="fname" name="oldimg" value="<?php echo $result['uploadedphography'];?>">
							</div>
                            <?php echo "<img src= 'uploadphography/".$result['uploadedphography']."' height='100px' width='150px' >";?>
                              <div>	
								 <button class="btn button" style="font-size: 19px; padding: 10px 25px; margin: 1px;" role="button"
								  name="editphotography">Edit Photography</button>   
								  </div>		
                             </form>
					  </div>
					</div>
...
if(isset($_POST['editphotography']))
{
...
{    
    ...
    $query = "UPDATE uploadphography SET firstname='$fname',lastname='$lname',yearofstudy='$year',
    department='$dept',uploadedphography='$filename' WHERE id='$id'";
