...
// error_reporting(0);
 ...
 $query = "SELECT * FROM getenglisharticle WHERE id='$id'";
...
<head>
  ...
  <body style="background-color:#fcf8f8;">	
           ...
           <button class="btn button" style="  font-size: 19px; padding: 10px 25px; margin: 1px;margin-top: 2%;"
            role="button" name="uploadenglisharticle">Upload English Article</button>  
        </form>
      </div>
    </div>
</body>			
...
if(isset($_POST['uploadenglisharticle']))
{
    ...
    $folder = "uploadengartiimg/".$filename;
   ...
   else
   {
    ...
    if($artiname !="" && $fname !="" && $lname !=""  && $year !="" && $dept !="" && $filename !="" && $article !="" && 
    $secarticle !="" && $thirdarticle !="" && $fullarticle !="")
{
    $query = "INSERT INTO uploadenglisharticle (articlename,firstname,lastname,yearofstudy,department,image,summery,
    summerysec,summerythird,englisharticle) VALUES ('$artiname','$fname','$lname','$year','$dept',
    '$filename','$article','$secarticle','$thirdarticle','$fullarticle')";
    ...
    if($data)
    {
        ...
        ?>
         <meta http-equiv = "refresh" content = "0; url = viewengartidata.php"/>
        <?php
    }
    else
    {
