<?php
 ...
 $query = "SELECT * FROM uploadmarathipoem WHERE id='$id'";
...
<a href="viewengpoemdata.php">
           ...
           <div>
            ...
            <?php echo "<img src= 'uploadmarathipoemimg/".$result['image']."' height='100px' width='150px' >";?>
            <div>
            <label for="Poem">Poem</label>
            <textarea id="Poem" name="poem">
            <?php echo $result['marathipoem'];?>
            </textarea>
            </div>
           <button class="btn button" style="font-size: 19px; padding: 10px 25px; margin: 1px;margin-top: 2%;" 
           role="button" name="editMpoem">Edit Marathi Poem</button> 
...
if(isset($_POST['editMpoem']))
{
    ...
    if($_FILES['newimg']['name'] != "")
    {
      ...
      $folder   = "uploadmarathipoemimg/".$filename;
   ...
   else
   {
    ...
    if($poemname !="" && $fname !="" && $lname !=""  && $year !="" && $dept !="" && $filename !="" && $Mpoem !="")
{
    $query = "UPDATE uploadmarathipoem SET poemname='$poemname',firstname='$fname',lastname='$lname',yearofstudy='$year',
    department='$dept',image='$filename',marathipoem='$Mpoem' WHERE id='$id'";
   ...
   if($data)
{
    ...
    ?>
       <meta http-equiv = "refresh" content = "0; url = uploadedmarathipoem.php"/>
    <?php
}
else
{        
