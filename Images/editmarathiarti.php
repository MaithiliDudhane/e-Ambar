...
<head>
  ...
  <a href="viewmarathiartidata.php">
           ...
           <div>
              ...
              <?php echo "<img src= 'uploadmarathiartiimg/".$result['image']."' height='100px' width='120px' >";?>
           ...
           <button class="btn button" style="  font-size: 19px; padding: 10px 25px; margin: 1px;margin-top: 2%;"
            role="button" name="editmarathiarticle">Edit Marathi Article</button>  
...
if(isset($_POST['editmarathiarticle']))
{
    ...
    if($_FILES['newimg']['name'] != "")
    {
      ...
      $folder = "uploadmarathiartiimg/".$filename;
   ...
   else
   {
    ...
    if($artiname !="" && $fname !="" && $lname !=""  && $year !="" && $dept !="" && $article !="" && $secarticle !="" && $thirdarticle !="" && $fullarticle !="")
{
    $query = "UPDATE uploadmarathiarticle SET articlename='$artiname',firstname='$fname',lastname='$lname',yearofstudy='$year',
    department='$dept',image='$filename',summery='$article',summerysec='$secarticle',
    summerythird='$thirdarticle',marathiarticle='$fullarticle' WHERE id='$id'";
   ...
   if($data)
{
    ...
    ?>
       <meta http-equiv = "refresh" content = "0; url = uploadedmarathiarticle.php"/>
    <?php
}
else
{        
